...
<form id="affwp-register-form" class="affwp-form" action="" method="post">
	...
	<fieldset>
		<legend class="mb-4"><?php _e( 'Register a new affiliate account', 'affiliate-wp' ); ?></legend>
		...
		<div class="form-group">
			...
			<input id="affwp-user-name" class="form-control form-control-lg" type="text" name="affwp_user_name" value="<?php if( ! empty( $user_name ) ) { echo $user_name; } ?>" title="<?php esc_attr_e( 'Your Name', 'affiliate-wp' ); ?>" <?php echo affwp_required_field_attr( 'your_name' ); ?> />
		</div>

		<div class="form-group">
			<label for="affwp-user-login"><?php _e( 'Username', 'affiliate-wp' ); ?></label>
			<input id="affwp-user-login" class="form-control form-control-lg" required="required" type="text" name="affwp_user_login" maxlength="60" value="<?php if( ! empty( $user_login ) ) { echo $user_login; } ?>" title="<?php esc_attr_e( 'Username', 'affiliate-wp' ); ?>"<?php echo $disabled; ?> />
		</div>

		<div class="form-group">
			<label for="affwp-user-email"><?php _e( 'Account Email', 'affiliate-wp' ); ?></label>
			<input id="affwp-user-email" class="form-control form-control-lg" required="required" type="email" name="affwp_user_email" value="<?php if( ! empty( $user_email ) ) { echo $user_email; } ?>" title="<?php esc_attr_e( 'Email Address', 'affiliate-wp' ); ?>"<?php echo $disabled; ?> />
		</div>

		<div class="form-group">
			<label for="affwp-payment-email"><?php _e( 'Payment Email', 'affiliate-wp' ); ?></label>
			<input id="affwp-payment-email" class="form-control form-control-lg" type="email" name="affwp_payment_email" value="<?php if( ! empty( $payment_email ) ) { echo $payment_email; } ?>" title="<?php esc_attr_e( 'Payment Email Address', 'affiliate-wp' ); ?>"<?php echo affwp_required_field_attr( 'payment_email' ); ?> />
		</div>

		<div class="form-group">
			<label for="affwp-user-url"><?php _e( 'Website URL', 'affiliate-wp' ); ?></label>
			<input id="affwp-user-url" class="form-control form-control-lg" type="text" name="affwp_user_url" value="<?php if( ! empty( $url ) ) { echo $url; } ?>" title="<?php esc_attr_e( 'Website URL', 'affiliate-wp' ); ?>" <?php echo affwp_required_field_attr( 'website_url' ); ?> />
		</div>

		<div class="form-group">
			<label for="affwp-promotion-method"><?php _e( 'How will you promote us?', 'affiliate-wp' ); ?></label>
			<textarea id="affwp-promotion-method" class="form-control" name="affwp_promotion_method" rows="5" cols="30"<?php echo affwp_required_field_attr( 'promotion_method' ); ?>><?php if( ! empty( $method ) ) { echo esc_textarea( $method ); } ?></textarea>
		</div>
		...
		<?php if ( ! is_user_logged_in() && isset( $required_registration_fields['password'] ) ) : ?>
			...
			<div class="form-group">
				...
				<input id="affwp-user-pass" required="required" class="password form-control form-control-lg" type="password" name="affwp_user_pass" />
			</div>

			<div class="form-group">
				<label for="affwp-user-pass2"><?php _e( 'Confirm Password', 'affiliate-wp' ); ?></label>
				<input id="affwp-user-pass2" required="required" class="password form-control form-control-lg" type="password" name="affwp_user_pass2" />
			</div>
		...
		<?php if ( ! empty( $terms_of_use ) ) : ?>
			<div class="form-group custom-control custom-checkbox">
				<input id="affwp-tos" class="custom-control-input" required="required" type="checkbox" name="affwp_tos" />
				<label class="affwp-tos custom-control-label" for="affwp-tos">
		...
		<div class="form-group">
			...
			<input class="btn btn-primary btn-lg" type="submit" value="<?php esc_attr_e( 'Register', 'affiliate-wp' ); ?>" />
		</div>
